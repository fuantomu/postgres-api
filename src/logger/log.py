...
def get_logger(class_name : str, level = get_log_level(getenv("SERVICE_LOG_LEVEL"))) -> logging.Logger:
    
    new_logger = logging.getLogger(class_name)
    if len(new_logger.handlers) > 0:
        return new_logger

    file_handler = logging.handlers.TimedRotatingFileHandler(f"logs/{class_name}.log", when='H', interval=12, backupCount=5)
