...
class Recipe(Router):
    ...
    def __init__(self):
        ...
        self.router.add_api_route("/ByIngredient", self.get_recipes_by_ingredient, methods=["GET"], status_code=202, summary="Get recipes that include a given ingredient", responses={400: {"model": BaseResponseModel}})
        self.router.add_api_route("/Ingredient", self.delete_recipe_ingredients, methods=["DELETE"], status_code=200,  summary="Delete one or more ingredient from a recipe", response_model=BaseResponseModel, responses={400: {"model": BaseResponseModel}})
    ...
    def get_recipes_by_ingredient(self, ingredient_id: str = None, ingredient: str = None, include_alias : bool = False) -> RecipeResponseModel | BaseResponseModel:
        ...
        return super().redirect_request('ByIngredient', {"key": "id" if ingredient_id else "name", "value": ingredient_id or ingredient, "include_alias": include_alias})
