...
class Recipe(Router):
    ...
    def __init__(self):
        ...
        self.router.add_api_route(
            ...
            methods=["POST"],
            status_code=201,
            summary="Add a new or update an existing recipe",
        ...
        self.router.add_api_route(
            ...
            methods=["GET"],
            status_code=202,
            summary="Get recipes that include a given ingredient",
            responses={400: {"model": BaseResponseModel}},
        )
        self.router.add_api_route(
            "/Ingredient",
            self.delete_recipe_ingredients,
            methods=["DELETE"],
            status_code=200,
            summary="Delete one or more ingredient from a recipe",
            response_model=BaseResponseModel,
            responses={400: {"model": BaseResponseModel}},
        )
    ...
    def get_recipes_by_ingredient(
    ...
    ) -> RecipeResponseModel | BaseResponseModel:
        self.logger.info(
            f"Received GET request on {self.name} - get_recipes_by_ingredient"
        )
        ...
        return super().redirect_request(
            "ByIngredient",
            {
                "key": "id" if ingredient_id else "name",
                "value": ingredient_id or ingredient,
    ...
    def delete_recipe_ingredients(
        self,
        ingredients: list[DeleteIngredientsModel],
        id: str = None,
        name: str = None,
    ):
        self.logger.info(
            f"Received DELETE request on {self.name} - delete_recipe_ingredients"
        )
