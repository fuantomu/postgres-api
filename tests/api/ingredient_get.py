...
class TestIngredientGet(BaseAPITest):
        ...
        for ingredient in self.ingredients:
            ...
            if response.status_code == self.get_success_code:
                ...
                for key in ingredient.keys():
                    self.assertEqual(response_ingredient[key],ingredient[key],f"{key} is not equal: {response_ingredient[key]} - {ingredient[key]}")
            else:
    ...
    def test_get_by_id_negative_incorrect_id(self):
        ...
        if response.status_code == self.bad_request_error_code:
            ...
            self.assertEqual(f"Value '{test_id}' does not match type of 'uuid'", response_ingredient)
        else:
    ...
    def test_get_by_name_positive(self):
        for ingredient in self.ingredients:
            response = self.client.get(f"/api/{self.endpoint}/?name={ingredient['name']}")
            if response.status_code == self.get_success_code:
                response_ingredient = response.json()["Result"][0] 
                for key in ingredient.keys():
                    self.assertEqual(response_ingredient[key],ingredient[key],f"{key} is not equal: {response_ingredient[key]} - {ingredient[key]}")
    ...
    def test_get_all(self):
        ...
        if response.status_code == self.get_success_code:
            self.assertEqual(len(self.ingredients),len(response.json()["Result"]),f"Did not return same length of result: {len(self.ingredients)} - {len(response.json()['Result'])}")
        else:
            self.fail(f"Did not get status code {self.get_success_code} - {response.status_code}")
    ...
    def test_get_by_name_search_alias_positive(self):
        ...
        for alias in ingredient['alias']:
            response = self.client.get(f"/api/{self.endpoint}/?name={alias}&search_alias=true")
            if response.status_code == self.get_success_code:
                response_ingredient = response.json()["Result"][0] 
                for key in ingredient.keys():
                    self.assertEqual(response_ingredient[key],ingredient[key],f"{key} is not equal: {response_ingredient[key]} - {ingredient[key]}")
    ...
    def test_get_by_id_search_alias_negative_unknown_name(self):
        test_name = 'Alias1000'
        response = self.client.get(f"/api/{self.endpoint}/?name={test_name}&search_alias=true")
        if response.status_code == self.get_success_code:
            response_ingredient = response.json()["Result"]
    ...
    def test_get_by_id_search_alias_positive(self):
        ingredient = self.ingredients[2]
        response = self.client.get(f"/api/{self.endpoint}/?id={ingredient['id']}&search_alias=true")
        if response.status_code == self.get_success_code:
            response_ingredient = response.json()["Result"][0] 
            for key in ingredient.keys():
                self.assertEqual(response_ingredient[key],ingredient[key],f"{key} is not equal: {response_ingredient[key]} - {ingredient[key]}")
    ...
    def test_get_by_id_search_alias_negative_incorrect_id(self):
        test_id = '12412414'
        response = self.client.get(f"/api/{self.endpoint}/?id={test_id}&search_alias=true")
        if response.status_code == self.bad_request_error_code:
            response_ingredient = response.json()["Result"]
            self.assertEqual(f"Value '{test_id}' does not match type of 'uuid'", response_ingredient)
        else:
