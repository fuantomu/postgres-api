...
class TestRecipesByIngredient(BaseAPITest):
    def __init__(self, methodName="runTest", client=None):
        ...
        for recipe in self.recipes:
            for ingredient in recipe["ingredients"]:
                ...
                self.ingredient_recipes[ingredient["name"]] = list(
                    set(self.ingredient_recipes[ingredient["name"]])
                )
    ...
    def test_get_negative_missing_id_name(self):
        ...
        if response.status_code == self.bad_request_error_code:
            self.assertEqual(
                "Missing one or more required parameters in: 'ingredient_id,ingredient'",
                response.text,
            )
        else:
            self.fail(
        ...
        for recipe in self.recipes:
            for ingredient in recipe["ingredients"]:
                response = self.client.get(
                    f"/api/{self.endpoint}/?ingredient={ingredient['name']}"
                )
                if response.status_code == self.get_success_code:
                    ...
                    self.assertEqual(
                        ...
                        len(self.ingredient_recipes[ingredient["name"]]),
                    )
                    self.assertIn(
                        recipe["name"], [_item["name"] for _item in response_recipes]
                    )
                    for response_recipe in response_recipes:
                        response_ingredient_names = [
                            _item["name"] for _item in response_recipe["ingredients"]
                        ]
    ...
    def test_get_by_id_positive(self):
        for recipe in self.recipes:
            for ingredient in recipe["ingredients"]:
                ...
                response = self.client.get(
                    f"/api/{self.endpoint}/?ingredient_id={ingredient_id}"
                )
                if response.status_code == self.get_success_code:
                    response_recipes = response.json()["Result"]
    ...
    def test_get_by_name_negative_unknown_name(self):
        ...
        if response.status_code == self.bad_request_error_code:
            self.assertEqual(
                f"No ingredient found for name '{test_name}'", response.json()["Result"]
            )
        else:
            self.fail(
                f"Did not get status code {self.bad_request_error_code} - {response.status_code}"
    ...
    def test_get_by_id_negative_unknown_id(self):
        ...
        if response.status_code == self.bad_request_error_code:
            self.assertEqual(
                f"No ingredient found for id '{test_id}'", response.json()["Result"]
            )
        else:
            self.fail(
                f"Did not get status code {self.bad_request_error_code} - {response.status_code}"
    ...
    def test_get_by_name_positive_include_alias(self):
        ...
        for alias_name in ingredient["alias"]:
            response = self.client.get(
                f"/api/{self.endpoint}/?ingredient={alias_name}&include_alias=true"
            )
            if response.status_code == self.get_success_code:
                response_recipes = response.json()["Result"]
                ...
                self.assertIn(
                    self.recipes[1]["name"],
                    [_item["name"] for _item in response_recipes],
                )
                for response_recipe in response_recipes:
                    response_ingredient_names = [
                        _item["name"] for _item in response_recipe["ingredients"]
    ...
    def test_get_by_name_negative_include_alias(self):
        ...
        if response.status_code == self.bad_request_error_code:
            response_recipe = response.json()["Result"]
            self.assertEqual(
                f"No ingredient found for alias '{alias_name}'", response_recipe
            )
        else:
            self.fail(
                f"Did not get status code {self.bad_request_error_code} - {response.status_code}"
