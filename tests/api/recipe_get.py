...
class TestRecipeGet(BaseAPITest):
        ...
        for recipe in self.recipes:
            ...
            if response.status_code == self.get_success_code:
                ...
                for key in recipe.keys():
                    self.assertEqual(response_recipe[key],recipe[key],f"{key} is not equal: {response_recipe[key]} - {recipe[key]}")
            else:
    ...
    def test_get_all(self):
        ...
        if response.status_code == self.get_success_code:
            self.assertEqual(len(self.recipes),len(response.json()["Result"]),f"Did not return same length of result: {len(self.recipes)} - {len(response.json()['Result'])}")
        else:
            self.fail(f"Did not get status code {self.get_success_code} - {response.status_code}")