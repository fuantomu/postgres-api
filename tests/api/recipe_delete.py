...
class TestRecipeDelete(BaseAPITest):
    ...
    def test_delete_by_id_positive(self):
        ...
        if response.status_code == self.base_success_code:
            ...
            self.assertEqual(
                f"Deleted recipe with id '{recipe['id']}'", response_delete
            )
            ...
            if response_recipes.status_code == self.bad_request_error_code:
                ...
                self.assertEqual(
                    f"No recipe found for id '{recipe['id']}'", result_recipes
                )
            else:
                self.fail(
    ...
    def test_delete_by_name_positive(self):
        ...
        if response.status_code == self.base_success_code:
            response_delete = response.json()["Result"]
            self.assertEqual(
                f"Deleted recipe with name '{recipe['name']}'", response_delete
            )
            response_recipes = self.client.get(f"/api/Recipe/?name={recipe['name']}")
            if response_recipes.status_code == self.bad_request_error_code:
                result_recipes = response_recipes.json()["Result"]
                self.assertEqual(
                    f"No recipe found for name '{recipe['name']}'", result_recipes
                )
            else:
                self.fail(
                    f"Did not get status code {self.bad_request_error_code} - {response_recipes.status_code}"
