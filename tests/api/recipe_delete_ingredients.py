...
class TestRecipeIngredientsDelete(BaseAPITest):
    def __init__(self, methodName="runTest", client=None):
        super().__init__(methodName, client)
        self.endpoint = "Recipe/RecipeIngredients"
    ...
    def test_delete_ingredient_by_name_positive(self):
        ...
        if response.status_code == self.base_success_code:
            ...
            self.assertEqual(f"Deleted ingredients '{','.join(ingredient['name'] for ingredient in request)}' for recipe with name '{recipe['name']}'", response_delete)
    ...
    def test_delete_ingredient_by_name_negative_unknown_name(self):
        ...
        response = self.client.request("DELETE",f"/api/{self.endpoint}/?name={recipe_name}", json=request)
        if response.status_code == self.bad_request_error_code:
            response_delete = response.json()["Result"]
            self.assertEqual(f"No recipe found for name '{recipe_name}'", response_delete)
        else:
    ...
    def test_delete_ingredient_by_id_positive(self):
        ...
        if response.status_code == self.base_success_code:
            response_delete = response.json()["Result"]
            self.assertEqual(f"Deleted ingredients '{','.join(ingredient['name'] for ingredient in request)}' for recipe with id '{recipe['id']}'", response_delete)
    ...
    def test_delete_ingredient_by_id_negative_incorrect_id(self):
        ...
        if response.status_code == self.bad_request_error_code:
            response_delete = response.json()["Result"]
            self.assertEqual(f"Value '{recipe_id}' does not match type of 'uuid'", response_delete)
        else:
            self.fail(f"Did not get status code {self.bad_request_error_code} - {response.status_code}")
    ...
    def test_delete_ingredient_negative_empty_request(self):
        ...
        if response.status_code == self.bad_request_error_code:
            response_delete = response.json()["Result"]
            self.assertEqual(f"No ingredients found in request", response_delete)
        else:
            self.fail(f"Did not get status code {self.bad_request_error_code} - {response.status_code}")
    ...
    def test_delete_ingredient_negative_bad_ingredient_id(self):
        ...
        if response.status_code == self.bad_request_error_code:
            response_delete = response.json()["Result"]
            self.assertEqual(f"No ingredients found for id '{request[0]['id']}'", response_delete)
        else:
            self.fail(f"Did not get status code {self.bad_request_error_code} - {response.status_code}")
    ...
    def test_delete_ingredient_positive_bad_ingredient_id(self):
        ...
        if response.status_code == self.base_success_code:
            response_delete = response.json()["Result"]
            self.assertEqual(f"Deleted ingredients '{request[1]['name']}' for recipe with name '{recipe['name']}'", response_delete)
            response_recipes = self.client.get(f"/api/Recipe/?id={recipe['id']}")
            if response_recipes.status_code == self.get_success_code:
                result_recipes = response_recipes.json()["Result"][0]
